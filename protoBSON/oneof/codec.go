...
// Codec for handling encoding and decoding of oneof fields. This default encoder is
...
//
// This means that while we can support oneof unions like [string, int], we cannot
...
func (codec *oneOfCodec) EncodeValue(
	...
	if err != nil {
		return fmt.Errorf(
			"could not get encoder for inner value type '%v' of oneof wrapper " +
				"type '%v': %w",
			innerValue.Type(),
			value.Type(),
			err,
	...
	if err != nil {
		return fmt.Errorf(
			"error encoding inner value type '%v' of oneof wrapper "+
				"type '%v': %w",
			innerValue.Type(),
			value.Type(),
			err,
...
func (codec *oneOfCodec) DecodeValue(
	...
	if bsonType == bsontype.Binary {
		...
		if err != nil {
			return fmt.Errorf("error getting binary subtype: %w", err)
		}
	}
	...
	if err != nil {
		return fmt.Errorf(
			"could not decode inner type '%v' of oneof type '%v': %w",
			innerValue.Type(),
			newWrapper.Type(),
			err,
		)
